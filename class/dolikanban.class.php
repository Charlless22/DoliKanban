...
/**
 * \file        class/dolikanban.class.php
 * \ingroup     dolikanban
 * \brief       This file is a CRUD class file for DoliKanban (Create/Read/Update/Delete)
 */
...
/**
 * Class for DoliKanban
 */
class DoliKanban extends SaturneObject
{
	/**
	 ...
	 */
	public $element = 'dolikanban';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'dolikanban_dolikanban';
	...
	/**
	 * @var string String with name of icon for dolikanban. Must be the part after the 'object_' into object_dolikanban.png
	 */
	public $picto = 'dolikanban@dolikanban';
    ...
    public $fields = [
        ...
        'amount_ht'       => ['type' => 'price', 'label' => 'DoliKanbanHT', 'enabled' => '1', 'position' => 80, 'notnull' => 0, 'visible' => 1, 'default' => 'null', 'isameasure' => '1'],
        'amount_ttc'      => ['type' => 'price', 'label' => 'DoliKanbanTTC', 'enabled' => '1', 'position' => 90, 'notnull' => 0, 'visible' => 1, 'default' => 'null', 'isameasure' => '1'],
    ...
    public function createFromClone(User $user, int $fromID): int
    {
        ...
        $doliKanbanID = $object->create($user);
	...
	public function getNomUrl(int $withpicto = 0, string $option = '', int $notooltip = 0, string $morecss = '', int $save_lastsearch_value = -1): string
	{
		...
		$label = $pictoDoliKanban.' <u>'.$langs->trans('DoliKanban').'</u>';
		...
		$url = dol_buildpath('/dolikanban/view/dolikanban_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (!empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER)) {
				$label = $langs->trans('ShowDoliKanban');
        ...
        if ($withpicto) $result .= $pictoDoliKanban . ' ';
        $result .= $linkstart;
        if ($withpicto != 2) {
            $result .= $this->ref;
        }
...
//		}
		...
		$hookmanager->initHooks(array('dolikanbandao'));
